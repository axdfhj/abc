...
def diy_make_person_rectangle_mask(shape,bbox):
    height, width = shape
    mask = np.zeros((height, width), np.float32)
    mask[round(bbox[1]):round(bbox[3]), round(bbox[0]):round(bbox[2])] = 1
    return mask[None, ...]

class diyPersonMaskGenerator:
    def __init__(self):
        return
    def __call__(self,img_shape,bbox_index):
        return diy_make_person_rectangle_mask(img_shape,bbox_index)
